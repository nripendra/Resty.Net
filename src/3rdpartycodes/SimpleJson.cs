            ...
            if (value is string)
            {
                ...
                if (!string.IsNullOrEmpty(str))
                {
                    if (type == typeof(DateTime) || (ReflectionUtils.IsNullableType(type) && Nullable.GetUnderlyingType(type) == typeof(DateTime)))
                    {
                        ...
                        if (DateTime.TryParseExact(str, Iso8601Format, CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal | DateTimeStyles.AdjustToUniversal, out parsedDateTime))
                        {
                            obj = parsedDateTime;
                        }
                        else if (str.StartsWith("/Date("))
                        {
                            ...
                            obj = new DateTime(1970, 1, 1, 0, 0, 0, 0).AddMilliseconds(long.Parse(str));
                        }
                    }
